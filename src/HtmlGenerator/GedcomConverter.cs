...
namespace HtmlGenerator
{
    ...
    {
        /// Create an xml file that contains the same hierarchy specified in the GEDCOM file. GEDCOM
        /// lines are limited to 255 characters, combineSplitValues indicates if the split lines
        /// should be combined into a single XML element.
        public static void ConvertToXml(string gedcomFilePath,
            string xmlFilePath, bool combineSplitValues)
        {
            // Store the previous level so can determine when need to close xml element tags.
            int prevLevel = -1;

            // Used to create the .xml file, XmlWriterSettings. Indent is specified if you want to
            // examine the xml file, otherwise it should be removed.
            ...
            using (XmlWriter writer = XmlWriter.Create(xmlFilePath, settings))
            {
                ...
                using (StreamReader sr = new StreamReader(gedcomFilePath))
                {
                    ...
                    {
                        // Some GEDCOM files indent each line with whitespace, delete any whitespace
        ...
        /// <summary>
        /// GEDCOM lines have a max length of 255 characters, this goes through the XML files and
        /// combines all of the split lines which makes the XML file easier to process.
        /// </summary>
    ...
    internal class GedcomLine
    {
        ...
        // Expression pattern used to clean up the GEDCOM tag. Tag can contain alphanumeric
        // characters, _, ., or -.
        ...
        /// <summary>
        /// Parse the Level, Tag, and Data fields from the GEDCOM line. The following is a sample
        /// GEDCOM line:
        ///
        ...
        public bool Parse(string text)
        {
            try
            {
                ...
                // The pointer reference is specified in the tag, and the tag in the data, swap these
                // two values to make it more consistent, the tag contains the tag and data contains
                ...
                if (this.tag[0] == '@')
                {
                    ...
                    // Some GEDCOM files have additional info, we only handle the tag info.
                    if (pos != -1)
